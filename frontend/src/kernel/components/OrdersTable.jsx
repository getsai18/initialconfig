// Debe calzar exactamente con Orden.status / Pedido.EN_PROGRESO/EN_PRODUCCION/TERMINADO
// del backend — ver OrderSummaryResponse.
const statusConfig = {
  'En progreso': { label: 'En Progreso', className: 'bg-blue-100 text-blue-800 border-blue-200' },
  'En producción': { label: 'En Producción', className: 'bg-yellow-100 text-yellow-800 border-yellow-200' },
  Terminado: { label: 'Terminado', className: 'bg-green-100 text-green-800 border-green-200' },
}
const DEFAULT_STATUS = { label: '—', className: 'bg-gray-100 text-gray-600 border-gray-200' }
        ...
        <table className="w-full">
          ...
          <tbody>
            {orders.map((order, index) => {
              const statusInfo = statusConfig[order.status] || DEFAULT_STATUS
              return (
