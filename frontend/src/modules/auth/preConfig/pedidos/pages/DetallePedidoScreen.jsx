...
import AdjuntoItem from '../../orders/components/AdjuntoItem';
import { ChevronLeft, Trash2, Layers, Plus, ChevronRight, Upload, Lock, CheckCircle, ClipboardList, AlertCircle, Ban, Copy, MinusCircle } from 'lucide-react';
...
export default function DetallePedidoScreen({
  ...
  const vencido = pedidoVencido(p.fechaLimite);
  const esCerrado = false;
  const puedoAgregar = !vencido && !esCerrado;

  const [modalAccion, setModalAccion] = useState({ isOpen: false, accion: null, tipo: null, id: '' });
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-start justify-between mb-6 flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-gray-900">{p.id}</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              className={`w-full text-xs px-2.5 py-1 bg-white border rounded-lg outline-none focus:ring-2 text-gray-900 transition-colors ${dateError ? 'border-red-400 focus:border-red-500' : 'border-gray-200'}`}
            />
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          ...
          <div className="text-xs text-gray-400 mt-0.5">{(p.ordenes || []).length} ordenes - {(p.kits || []).length} kits</div>
        </div>
      </div>

      <div className="flex justify-end mb-4">
        {!p.confirmado ? (
          <button onClick={onConfirmarPedido} className="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold bg-[#00b853] text-white rounded-lg hover:bg-emerald-600 transition-colors shadow-sm">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
        ...
        {ordenesActivasSueltas.length === 0 ? (
          <div className="text-center py-8 text-gray-400">
            ...
            <div className="text-sm">Sin órdenes activas</div>
          </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
        ...
        {kitsActivos.length === 0 ? (
          <div className="text-center py-8 text-gray-400">
            ...
            <div className="text-sm">Sin kits activos</div>
          </div>
        ) : kitsActivos.map((k) => (
          <KitBlock
            key={k.id}
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-sm w-full p-5 border border-gray-100">
            ...
            <div className="flex items-center justify-end gap-3">
              ...
              <button
                ...
                className={`px-4 py-2 text-sm font-medium text-white rounded-lg ${modalConfig.btnColor}`}
              >
