...
import { getDoc } from '../../app/storage'
...
export function ConfirmacionesMaterial() {
  ...
  const fetchData = () => {
    Promise.all([
      getDoc('cp_v5_entregas', []),
      getDoc('cp_v5_finalizaciones', []),
    ])
      .then(([savedEntregas, savedFinalizaciones]) => {
        if (savedEntregas) setEntregas(savedEntregas)
        if (savedFinalizaciones) setFinalizaciones(savedFinalizaciones)
      })
      .catch(console.warn)
  ...
  // Escuchar cambios en PouchDB en tiempo real
  useEffect(() => {
    const feed = db.changes({ live: true, since: 'now', include_docs: true })
      .on('change', (change) => {
        if (change.id === 'cp_v5_entregas' && change.doc?.data) {
          setEntregas(change.doc.data)
        }
        if (change.id === 'cp_v5_finalizaciones' && change.doc?.data) {
          setFinalizaciones(change.doc.data)
        }
      })
    return () => feed.cancel()
  }, [])

  // Normalizar y unificar los datos en una sola lista para el historial
  const items = [
    ...entregas.map(e => ({
      id: e.id,
      tipo: 'entrega',
  ...
  // Obtener listado de áreas de origen para el filtro
  ...
  return (
    <div className="p-8">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <button 
          onClick={fetchData} 
          className="p-2 hidden rounded-lg border border-border text-muted-foreground hover:text-foreground hover:bg-accent transition-colors flex items-center gap-2 text-sm"
