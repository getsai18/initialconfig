...
import { getDoc, setDoc } from '../../app/storage'
...
export function Incidencias({ isSubAdmin: propIsSubAdmin, incidencias: externalIncidencias, setIncidencias: externalSetIncidencias } = {}) {
  ...
  useEffect(() => {
    ...
    getDoc('cp_v5_incidencias', [])
      .then(saved => {
        if (saved && Array.isArray(saved)) setLocalIncidencias(saved)
      })
      .catch(console.warn)
  ...
  useEffect(() => {
    if (usesExternalState || !loadedRef.current) return
    setDoc('cp_v5_incidencias', localIncidencias).catch(console.warn)
  }, [localIncidencias, usesExternalState])
          ...
          <div className="bg-card border border-border rounded-xl shadow-xl w-full max-w-md mx-4 p-6 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-4 text-sm">
              <div className="grid grid-cols-2 gap-2 bg-muted/30 p-3 rounded-lg border border-border">
                ...
                {detalleModal.severidad && (
                  <div>
                    <span className="text-xs text-muted-foreground block">Severidad</span>
                    <span className="font-medium">{detalleModal.severidad}</span>
                  </div>
                )}
              </div>
