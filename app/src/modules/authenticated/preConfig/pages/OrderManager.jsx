...
import IncidenciasGestorScreen from '../components/IncidenciasGestorScreen';
...
import {
  ...
  LS_KEY, LS_KEY_OIDS } from
'../data/catalogos';
import { deleteDoc, getDoc, setDoc } from '../storage';
import { db } from '../db';
import GestorSidebar from '../components/GestorSideBar';
import { ConfirmacionesMaterial } from '../admin/views/ConfirmacionesMaterial';
import {
  pedidoVencido, tipoSolicitudLimpio,
  generarOrdenDisplayId, generarOrdenInternalCode,
  disciplinaAbrev } from
'../utils/helpers';
...
export default function OrderManager({ onLogout = null }) {
  ...
  function eliminarOrden(ordenId) {
    updatePedido(clienteActivoId, pedidoActivoId, (p) => ({
      ...p,
      // Filtramos (borramos) la orden de la lista principal
      ordenes: p.ordenes.filter((o) => o.id !== ordenId),
      // También la quitamos de cualquier kit que la contenga
      kits: p.kits.map((k) => ({
        ...k,
  ...
  function confirmarAgregarOrdenesKit(kitId, selectedOrdenIds) {
    ...
    updatePedido(clienteActivoId, pedidoActivoId, (p) => ({
      ...
      kits: p.kits.map((k) => {
        ...
        // Quita las órdenes movidas de cualquier otro kit que las tuviera
  ...
  // Catálogo real de actividades (el mismo que usa ActividadesModal al agregar actividades a
  // un área), necesario para poder resolver por nombre las actividades guardadas de una orden
  // al reutilizarla. `getCatalogoActividad`/`catalogoActividades` es un catálogo legado que ya
  // no coincide con lo que el admin gestiona en `cp_v5_actividades`.
  ...
  // Duplica una orden existente (suelta o dentro de un kit) como punto de partida
  // de una orden nueva: si `kitId` viene de una orden que vivía en un kit, la copia
  // se une a ese mismo kit al confirmarse (igual que "+ Nueva" dentro del kit).
  ...
  function initNuevaOrden(reutilizar, ordenId, baseAreas, actividadesCatalog) {
    ...
    if (reutilizar) {
      // Al reutilizar una orden concreta, el borrador solo debe partir de los
      // adjuntos que esa orden ya tenía — no del pool de adjuntos del pedido
      // (que puede haber crecido después de creada la orden original).
      ...
      setAreasActivas(areasBase.map((a) => {
        ...
        if (savedArea) {
          const actividades = savedArea.actividades.map((sa) => {
            const def = (actividadesCatalog || []).find((c) => c.nombre === sa.actividad) || getCatalogoActividad(sa.actividad);if (!def) return null;
  ...
  function _ejecutarConfirmarOrden() {
    // New ID format: ORD-DDMMYYYY-NNNN (display) / ORD-DDMMYYYY-NNNN-DISC-Tipo (internal)
  ...
  function handleAdjuntos(event) {
    ...
    files.forEach((f) => {
      ...
      reader.onload = (e) => {
        ...
        // Solo afecta al borrador de esta orden; el pool general del pedido no se toca aquí.
    ...
    <div className="flex flex-col md:flex-row min-h-screen bg-gray-50 text-gray-900 font-sans">
      ...
      <div className="flex flex-col flex-1 min-w-0 h-screen md:h-auto overflow-y-auto md:overflow-visible">
        ...
        <div className="flex-1 py-8 px-8 max-w-6xl w-full mx-auto">
          ...
          {screen === 'incidencias' &&
          <IncidenciasGestorScreen incidencias={incidencias} />
          }
          {screen === 'confirmaciones' &&
          <ConfirmacionesMaterial />
          }
          {screen === 'nueva-orden' &&
          <NuevaOrdenScreen
            pedidoActivo={pedidoActivo}
