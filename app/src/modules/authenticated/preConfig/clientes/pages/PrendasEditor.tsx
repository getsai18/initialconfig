...
const genLabel: Record<string, string> = { hombre: 'Hombre', mujer: 'Mujer', niño: 'Niño' };
...
interface Props {
  ...
  // Tipos de prenda ofrecidos en el selector "Tipo de prenda" del formulario de alta.
  tiposDisponibles: string[];
  // Oculta el formulario de alta y el botón de quitar — solo muestra las tarjetas.
  readonly?: boolean;
  // Cantidad mínima de prendas que deben permanecer en la orden; si al quitar una
  // se llegaría por debajo de este límite, el botón de quitar se deshabilita.
  minItems?: number;
}

// Formulario para agregar prendas (tipo/género/talla/cantidad) + grid de prendas ya
// agregadas con opción de quitar. Usado tanto en el wizard de Nueva Orden como en la
// edición de una orden ya existente (OrdenDetalleModal), sobre el mismo shape de datos:
// { id, name, conf: { type, size, tot } }.
export default function PrendasEditor({ clothesData, onChange, tiposDisponibles, readonly, minItems = 0 }: Props) {
  ...
  const [pendingRemoveClothId, setPendingRemoveClothId] = useState<string | null>(null);
  ...
  function agregarCloth() {
    ...
    const prendaActiva = prendasCatalogo.find((p: any) => p.nombre === clothTipo);
    ...
    const existente = clothesData.find((c: any) => c.name === clothTipo && c.conf.type === generoSeleccionado && c.conf.size === tallaFinal);
    if (existente) {
      onChange(clothesData.map((c: any) =>
  ...
  function handleQuitarCloth(id: string) {
    if (clothesData.length <= minItems) return;
    if (pendingRemoveClothId === id) {
      onChange(clothesData.filter((c: any) => c.id !== id));
  ...
  function handleAjustarCantidad(id: string, delta: number) {
    onChange(clothesData.map((c: any) => {
      if (c.id !== id) return c;
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
        ...
        <div className="mb-3">
          ...
          <select value={clothTipo} onChange={e => setClothTipo(e.target.value)} className="field-input">
            ...
            {tiposDisponibles.map((p: string) => <option key={p} value={p}>{p}</option>)}
          </select>
        </div>
        <div className="mb-3">
        ...
        {(() => {
          const prendaActivaUI = prendasCatalogo.find((p: any) => p.nombre === clothTipo);
          ...
          return (
            <div className="mb-3">
              ...
              <select value={clothTalla} onChange={e => setClothTalla(e.target.value)} className="field-input">
                <option value="">Seleccionar…</option>
                {tallaOptions.map((t: string) => <option key={t} value={t}>{t}</option>)}
              </select>
            </div>
          );
        })()}
