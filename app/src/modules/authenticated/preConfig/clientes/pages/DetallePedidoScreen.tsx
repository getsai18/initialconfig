...
function OrdenRow({ o, onVerDetalle, esBorrador, onPedirAccion, puedoAgregar, onReutilizar }: any) {
  const isCanceled = o.status === 'Cancelado';
  const totalPiezas = (o.clothes || []).reduce((s: number, c: any) => s + (c.conf.tot || 0), 0);
...
function OrdenRowKit({ o, kitId, onVerDetalle, esBorrador, onPedirAccion, kitCancelado, puedoAgregar, onReutilizar, onQuitarDelKit }: any) {
...
function KitBlock({ k, pedidoActivo, puedoAgregar, onVerDetalle, onAbrirOrdenesKit, onCrearOrdenKit, onPedirAccion, onReutilizar, onQuitarDelKit }: any) {
  const isCanceled = k.status === 'Cancelado';
  const ordenesDelKit = k.ordenIds.map((oid: string) => pedidoActivo.ordenes.find((o: any) => o.id === oid)).filter(Boolean);
...
export default function DetallePedidoScreen({
  ...
  const [modalAccion, setModalAccion] = useState<{ isOpen: boolean, accion: 'eliminar' | null, tipo: 'kit' | 'orden' | null, id: string }>({ isOpen: false, accion: null, tipo: null, id: '' });

  function handlePedirAccion(accion: 'eliminar', tipo: 'kit' | 'orden', id: string) {
  ...
  const ordenesEnKit = new Set(p.kits.flatMap((k: any) => k.ordenIds));
  const ordenesSueltas = p.ordenes.filter((o: any) => !ordenesEnKit.has(o.id));

  const ordenesActivasSueltas = ordenesSueltas.filter((o: any) => o.status !== 'Cancelado');
  const ordenesCanceladasSueltas = ordenesSueltas.filter((o: any) => o.status === 'Cancelado');

  const kitsActivos = p.kits.filter((k: any) => k.status !== 'Cancelado');
  const kitsCancelados = p.kits.filter((k: any) => k.status === 'Cancelado');
  ...
  return (
    <div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
        ...
        ) : ordenesActivasSueltas.map((o: any) => (
