...
export function LoginForm({ onLogin, onForgotPassword, onFirstLogin }) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const role = payload.usuario?.role || payload.role
      const area = payload.usuario?.areaNombre || payload.areaNombre || null
      onLogin?.(role, area)
    } catch (error) {
      console.error(error)
  ...
  return (
    <div className="bg-card border border-border rounded-lg p-8 shadow-lg">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <button type="submit" disabled={isSubmitting} className="w-full bg-primary text-primary-foreground py-2.5 rounded-lg hover:opacity-90 transition-opacity disabled:opacity-60 disabled:cursor-not-allowed">
