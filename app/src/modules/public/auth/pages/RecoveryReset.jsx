...
export function RecoveryReset({ matchedUser, setUsuarios, onSuccess, onBack }) {
  ...
  const handleResetPasswordSubmit = (data) => {
    ...
    if (setUsuarios) {
      setUsuarios(prev => prev.map(u => 
        u.id === matchedUser.id ? { ...u, password: data.nuevaPassword } : u
      ))
    } else {
      matchedUser.password = data.nuevaPassword
    }
  ...
  return (
    <div className="bg-card border border-border rounded-lg p-8 shadow-lg">
      ...
      <form onSubmit={handleSubmit(handleResetPasswordSubmit)} className="space-y-5">
        ...
        <div>
          <button type="button" onClick={onBack} className="w-full bg-primary flex-1 bg-secondary text-secondary-foreground py-2 rounded-lg text-sm hover:opacity-90 transition-opacity border border-border">
            Volver al Login
          </button>
        </div>
      </form>
